...
// Serve static files from backgrounds directory
router.use('/backgrounds', express.static(path.join(__dirname, 'backgrounds')));
...
router.get("/generate", async (req, res) => {
    try {
        // Generate ranked image using parameters
        ...
        res.status(500).json({ 
            error: 'Error generating ranked image',
    ...
    try {
        ...
        if (background && background !== 'none') {
            try {
                ...
                if (!isNaN(background)) {
                    // Numbered background
                    const bgPath = path.join(__dirname, 'backgrounds', `bg${background}.png`);
